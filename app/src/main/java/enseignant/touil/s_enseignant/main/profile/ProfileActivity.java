package enseignant.touil.s_enseignant.main.profile;
...
import enseignant.touil.s_enseignant.main.database.DatabaseHandler;
...
public class ProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textView.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                ...
                alert.setTitle("Supprimer l'élève");
                alert.setMessage("Êtes-vous sûr ?");
                alert.setPositiveButton("Oui", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (AppBase.handler.execAction(qu)) {
                            Log.d("supprimer", "fait de l'étudiant");
                            String qa = "DELETE FROM ATTENDANCE WHERE register = '" + regno.toUpperCase() + "'";
                            if (AppBase.handler.execAction(qa)) {
                                Toast.makeText(getBaseContext(), "Effacé", Toast.LENGTH_LONG).show();
                                Log.d("supprimer", "fait de la presence");
                            }
                        }
                    }
                });
    ...
    public void find(View view) {
        ...
        if (cursor == null || cursor.getCount() == 0) {
            assert textView != null;
            textView.setText("Pas de données disponibles");
        } else {
            String attendance = "";
            if (att < 0) {
            ...
            if (cursorx == null || cursorx.getCount() == 0) {
                Toast.makeText(getBaseContext(), "Aucune information de présence disponible", Toast.LENGTH_LONG).show();
            } else {
                cursorx.moveToFirst();
